# INDU Brain v2.1 - Deployment Package

**Version**: 2.1
**Release Date**: 2026-01-13
**Description**: Production-ready INDU voice assistant with RAG (Retrieval Augmented Generation)
...
- `indu_rag.py` - RAG engine with FAISS vector search
- `indu_system_prompt.txt` - INDU personality and behavior rules
- `indu_knowledge_base.txt` - SSi medical robotics knowledge base
...
### To update INDU's knowledge:

1. **Edit knowledge base**
```bash
nano indu_knowledge_base.txt
```

2. **Rebuild vectorstore**
```bash
rm -rf indu_vectorstore
python setup.py
```
...
### Issue: "No module named 'indu_rag'"
...
### Issue: "Empty responses from INDU"
...
├── indu_rag.py                    # RAG engine
├── indu_system_prompt.txt         # INDU personality
...
│   ├── README.md
│   └── QUICKSTART.md
└── indu_vectorstore/             # Created on first run
    └── [FAISS index files]
```
...
4. Increase `chunk_size` in indu_rag.py for faster retrieval
...
- Improved INDU identity distinction
