 ...
 * Same functionality as original enroll.cpp
 * Works independently without ROS2
 */
...
struct EnrollConfig {
    // Run from workspace root, or specify paths via CLI
...
bool captureFromCamera(const std::string& name, const EnrollConfig& config) {
    ...
    while (true) {
        ...
        // Resize for display (720p) while keeping full res for detection
        cv::Mat display;
        cv::resize(frame, display, cv::Size(1280, 720));
        cv::imshow("Enroll: " + name, display);
