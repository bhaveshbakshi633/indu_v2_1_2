...
const int32_t ROBOT_API_ID_LOCO_SET_CUSTOM_ACTION = 7108;
...
class LocoClient {
 ...
 public:
  ...
  // Execute custom recorded action by name (API 7108)
  int32_t ExecuteCustomAction(const std::string& action_name) {
    unitree_api::msg::Request req;
    req.header.identity.api_id = ROBOT_API_ID_LOCO_SET_CUSTOM_ACTION;
    nlohmann::json js;
    js["action_name"] = action_name;
    req.parameter = js.dump();
    return base_client_.Call(req);
  }
  ...
  // Custom recorded actions (API 7108)
  int32_t Headshake() { return ExecuteCustomAction("Headshake"); }
  int32_t Hug() { return ExecuteCustomAction("HUG"); }
  int32_t Hii() { return ExecuteCustomAction("Hii"); }
